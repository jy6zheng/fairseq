...
DEFAULT_MAX_SOURCE_POSITIONS = 101
...
class PositionTransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # args for position attention function
        parser.add_argument('--positional-encoding', default=False, action='store_true',
                            help='add absolute position to input word encoding')
        parser.add_argument('--positional-layers', nargs="+", type=int)
        # fmt: on
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return PositionTransformerEncoder(args, src_dict, embed_tokens)
...
def position_attention(x, pos_table, sf_type):
    # target table is a matrix B x T(# words) x C that is the target position for each word

    # T x B x C -> B x T x C
    x = x.transpose(0, 1)
    dim = x.size(-1)
    # pos_table : B x T x C -> B x C x T, scores: B x T (# words) x T (# positions)
    ...
    # [B x T (words) x T (positions)] x [T(positions) x C] -> B x T x C
...
class PositionTransformerEncoder(FairseqEncoder):
    """
    ...
    def forward_embedding(
        ...
        if self.embed_positions is not None and self.positional_encoding:

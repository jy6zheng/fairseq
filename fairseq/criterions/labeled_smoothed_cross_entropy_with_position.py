...
@register_criterion("label_smoothed_cross_entropy_with_position")
class LabelSmoothedCrossEntropyCriterionWithPosition(
    ...
    def compute_alignment_loss(self, sample, net_output, probability):
        ...
        for layer in probability:
            ...
            if len(align) > 0:
                loss = -(
                    (prob[align[:, 1][:, None], align[:, 0][:, None]]).log() * align_weights[:, None]
                ).sum()
                total_loss += loss
            else:
                return None
